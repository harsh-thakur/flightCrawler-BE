...
exports.getCities = async (req,res) => {
  ...
  request({
    method: 'GET',
    uri:`https://api.sandbox.amadeus.com/v1.2/airports/autocomplete?apikey=${process.env.API_KEY}&term=${cityName}&country=IN`
...
// const apikey = "tCh0GDqllwJiVaWuuKEutSiSgd2KG6Xq"
    ...
    if(preference == "all"){
      ...
      setTimeout(function(){
        let url = `https://api.sandbox.amadeus.com/v1.2/flights/low-fare-search?apikey=${process.env.API_KEY}&origin=${originValue}&destination=${destinationValue}&departure_date=${d}&nonstop=${opt}&currency=INR`
        request({
              method: 'GET',
              uri: url
            }, function (error, response, body){
      ...
      setTimeout(function(){
        let url = `https://api.sandbox.amadeus.com/v1.2/flights/low-fare-search?apikey=${process.env.API_KEY}&origin=${originValue}&destination=${destinationValue}&departure_date=${d}&include_airlines=${preference}&nonstop=${opt}&currency=INR`
        request({
              method: 'GET',
              uri: url
            }, function (error, response, body){
